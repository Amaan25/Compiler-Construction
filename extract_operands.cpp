...
void slim::IR::dumpIR()
{
    ...
    for (auto &entry : this->func_bb_to_inst_id)
    {
        ...
        for (long long instruction_id : entry.second)
        {
            ...
            llvm::outs() << ins_type[instruction->getInstructionType()] << " instruction" << "\n";

            unsigned ops = instruction->getNumOperands();
            if (ops > 0) {
                llvm::outs() << "Number of Operands: " << ops << "\n";
                if (instruction->getResultOperand().first) {
                    llvm::outs() << "Operands in LHS: " << instruction->getLHS().first->getName() << "\n";
                }
                llvm::outs() << "Operands in RHS: ";
                for (unsigned i = 0; i < ops; i++) {
                    llvm::outs() << instruction->getOperand(i).first->getName() << " ";
                }
                llvm::outs() << "\n";
                if (instruction->getInstructionType() == InstructionType::BINARY_OPERATION) {
                    BinaryOperation *b = (BinaryOperation *) instruction;
                    llvm::outs() << "Operator: " << op_type[b->getOperationType()] << "\n";
                }
            }
        }
